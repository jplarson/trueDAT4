...
var HistoryManager = {
	 ...
	 * iframeSrc: Src for IE6/7 iframe, must exist on server!
...
//		alert (defaults);
	...
	unregister: function(key) {
		this.fireEvent('onUnregister', [key]);
		this.modules.erase(key);
	},
	...
	generate: function(key, values) {
		...
		data.values = current;
		return state.length ? '#' + state  :  '#_';
	},

	observe: function() {
		if (this.timeout) return;
		var state = this.getState();
	//	dbug.log(this.state + ' vs ' + state);
		...
		this.modules.each(function(data, key) {
			...
			// jpl Change to trigger action on default state.  Was:	
				if (!bits.isSimilar(data.defaults)) data.values = bits;
			// Now:
			//	data.values = bits;
			} else data.values = $A(data.defaults);
		//	if(!data.skipDefaultMatch  ||  !data.values.isSimilar(data.defaults)) // jpl hack: make this conditional to honor skipDefaultMatch
	...
	getHash: function() {
		var href = location.href;  // jpl remove top. to allow nesting
	...
	$extend(HistoryManager, Events.prototype);
	$extend(HistoryManager, Options.prototype);
